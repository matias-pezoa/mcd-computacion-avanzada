 ...
 *
 *   1) ADXL345 (acelerómetro) -> eje Y convertido a ángulo de inclinación
 *      (-90 a 90°). Se publica el ángulo continuo + una bandera
 *      "cambioBrusco" que se activa cuando la inclinación salta más de
 *      UMBRAL_CAMBIO_BRUSCO grados entre dos lecturas consecutivas
 *      (pensado para disparar el glitch/fragmentación en la web).
 *
 ...
 *
 * Payload (ESQUEMA NUEVO — distinto al de instrumento_esp32.ino):
 *   {
 ...
 *   }
 *
 * web/main.js ya lee este esquema (inclinacion/cambioBrusco/controlValor/
 * colorHex): inclinación -> ruido, cambioBrusco -> fragmentación, el dial
 * del potenciómetro escala ambos, y colorHex se aplica como filtro de
 * color sobre el video. Ver "04 — REGLAS" en main.js para el detalle.
 *
 * -----------------------------------------------------------------
...
void actualizarPantalla(float accelY, float inclinacion, bool cambioBrusco,
...
void loop() {
  ...
  /* --- 1. Acelerómetro: inclinación en el eje Y --- */
  float accelY = 0.0f, inclinacion = 0.0f;
  ...
  if (accelOk) {
    ...
    // Componente de gravedad en Y -> ángulo respecto de la horizontal.
    float proporcion = constrain(accelY / 9.80665f, -1.0f, 1.0f);
    inclinacion = degrees(asin(proporcion)); // -90 a 90
  ...
  snprintf(payload, sizeof(payload),
    "{\"clientId\":\"%s\",\"timestamp\":%lu,\"accelY\":%.2f,\"inclinacion\":%.1f,"
  ...
  Serial.printf("accelY=%.2f inc=%.1f brusco=%s | color=%s (%s) | pot=%.0f\n",
                accelY, inclinacion, cambioBrusco ? "SI" : "no", colorHex, dominante, potSuave);
}
